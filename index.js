...
function readWidgetConfig() {
  for (const base of [PACKAGE_ROOT, path.dirname(fileURLToPath(import.meta.url))]) {
    try { return JSON.parse(fs.readFileSync(path.join(base, 'ergouzi.config.json'), 'utf8')) || {} } catch {}
  }
  return {}
}
...
const ERGOUZI_AGENT_HOST = process.env.ERGOUZI_BIND_HOST || WIDGET_CONFIG.bindHost || '127.0.0.1'
const ERGOUZI_AGENT_PORT = Number(process.env.ERGOUZI_AGENT_PORT || WIDGET_CONFIG.agentPort || 17891)
const ERGOUZI_AGENT_URL = process.env.ERGOUZI_AGENT_URL || ('http://' + ERGOUZI_AGENT_HOST + ':' + ERGOUZI_AGENT_PORT)
const BALANCE_TTL_MS = Number(process.env.ERGOUZI_BALANCE_TTL_MS || WIDGET_CONFIG.balanceTtlMs || 25000)
...
var STEP = 0.1
var CLICK_SQ = 9
var REFRESH_MS = Number(process.env.ERGOUZI_WIDGET_REFRESH_MS || WIDGET_CONFIG.widgetRefreshMs || 60000)
var CHANGE_MS = 900
var ANIM_MS = 700
var BUBBLE_MS = Number(process.env.ERGOUZI_WIDGET_BUBBLE_MS || WIDGET_CONFIG.widgetBubbleMs || 5000)
var FETCH_TIMEOUT_MS = Number(process.env.ERGOUZI_REQUEST_TIMEOUT_MS || WIDGET_CONFIG.requestTimeoutMs || 25000)
